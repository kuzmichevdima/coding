...
inline void insert(Pt & root, int x0)
{
    Pt A, B, tmp;
    split(root, x0, A, B);
    //printf("split x0 = %d A.sz = %d B.sz = %d\n", x0, get_sz(A), get_sz(B));
    Pt C = new Treap(x0);
    merge(tmp, A, C);
    //printf("after merge tmp.sz = %d\n", get_sz(tmp));
    merge(root, tmp, B);
}
...
void print(Pt T)
{
    if (T == NULL)
        return;
    //printf("x = %d y = %d L = %d R = %d sz = %d\n", T->x, T->y, (T->l == NULL ? -1 : T->l->x), (T->r == NULL ? -1 : T->r->x), T->sz);
    print(T->l);
    print(T->r);
}
...
int main() {
    ...
    forn(j, n)
    {
        int t;
        scanf("%d", &t);
        /*printf("iteration %d: root_plus: ", j);
        print(root_plus);
        printf("\nroot_minus: ");
        print(root_minus);
        printf("\n");*/
