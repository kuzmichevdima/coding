...
void dfs(int v, int anc, vector<vi>& edges) {
    h[v] = anc == -1 ? 0 : h[anc] + 1;
    jmp[v][0] = anc;
    fore(j, 1, LOG) {
        int x = jmp[v][j - 1];
...
int count_dfs(int v, int anc, int top, const vector<vi>& edges) {
    int sum = 0;
    ...
    for (int u : edges[v]) {
        if (u == anc)
            continue;
        int cur = count_dfs(u, v, new_top, edges);
        part += (i64)cur * sum;
        sum += cur;
    }
