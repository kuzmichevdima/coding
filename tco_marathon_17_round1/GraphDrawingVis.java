...
// ------------- visualization part ----------------------
    ...
    public GraphDrawingVis(String seed) {
        ...
        score = runTest(seed);
        System.out.println("Test " + seed + " Score = " + score + "\n");
        if (proc != null)
            try { proc.destroy(); } 
            catch (Exception e) { e.printStackTrace(); }
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i<args.length; i++) {
            ...
            if (args[i].equals("-seed") && i + 1 < args.length)
                chosen_seed = args[i + 1];
        }
        int seed0, seed1;
        if (chosen_seed != "-1") {
            seed0 = Integer.parseInt(chosen_seed);
            seed1 = seed0;
        } else {
            seed0 = 2;
            seed1 = 20;
        ...
        for (int seed = seed0; seed <= seed1; seed++) {
            GraphDrawingVis f = new GraphDrawingVis(Integer.toString(seed));
            scores_sum += f.score;
        }
        System.out.println("Average score = " + scores_sum * 1e6 / (seed1 - seed0 + 1));
    }
    // ---------------------------------------------------
