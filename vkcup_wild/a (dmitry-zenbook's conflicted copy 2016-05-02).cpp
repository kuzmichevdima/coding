...
inline bool urgent(int sol) {
    //return list[sol].curTest - list[sol].testsComplete <= max(1, invokers / 2);
    //return true;
    return pull.empty() || list[sol].curTest - list[sol].testsComplete <= invokers / (int)pull.size() / (int)pull.size();
}
...
void readResults() {
    while(true) {
        ...
        if (verdict != "OK" && !list[sol].counted) {
           // if (DEBUG)
                fprintf(stderr, "sol = %d testsCnt = %d runs = %d failing_test = %d wasted = %d\n", sol, task[taskNum].testsCnt, list[sol].curTest, list[sol].testsComplete,
                        list[sol].curTest - list[sol].testsComplete);
        ...
        //else list[sol].average = task[taskNum].average * (task[taskNum].testsCnt - list[sol].curTest) / task[taskNum].testsCnt;
            ...
            /*if (list[sol].average <= 0)
                list[sol].average = task[taskNum].testsCnt - list[sol].curTest;*/
        }
        list[sol].average = max(1, list[sol].average);
        if (DEBUG)
            fprintf(stderr, "sol = %d complete = %d average = %d averages.back() = %d averages[%d] = %d\n", sol, complete, list[sol].average, task[taskNum].averages.back(), complete,
...
bool add_query(int sol) {
    ...
    list[sol].curTest++;
    t--;
    if (list[sol].curTest == task[list[sol].taskNum].testsCnt || (!urgent(sol))) {
        pull.erase(mp(list[sol].average, sol));
        return true;
    } else return false;
}
