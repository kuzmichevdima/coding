...
vector <int> breaking_test = {
  4, 4, 4, 4, 4, 4,  7, 7, 6, 7, 6, 7, 6, 7, 6, 7, 6, 6, 7, 6, 7, 6, 7,  6, 7, 6, 7, 6, 7, 6, 7, 6,   7,  1, 1, 1, 3, 3, 3, 3, 3, 3 
};
...
int main(int argc, char * argv[]) {
    ...
    //forn(j, breaking_test.size())
      //  add(breaking_test[j]);
    while((int)a.size() < n || test_type == NO)
    {
        int pos = a.size();
        if (a.back() == 7 && rnd.next(1, 8) == 1 && (int)a.size() + 500 < n)
        {
               forn(j, 5)
			       add(7);
				forn(it, rnd.next(10, 50))
				{
					add(6);
					if (rnd.next(1, 2) == 1)
					{
        ...
        else if (a.back() == breaking_test[0] && rnd.next(1, 8) == 1 && (int)a.size() + 500 < n)
        {
            forn(j, breaking_test.size())
                add(breaking_test[j]);
            continue;
        }
        else if (a.back() == 1 && rnd.next(1, 8) == 1 && (int)a.size() + 500 < n)
        {
				forn(j, 5)
			       add(1);
				forn(it, rnd.next(10, 50))
